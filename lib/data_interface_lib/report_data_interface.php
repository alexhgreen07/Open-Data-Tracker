...
class Report_Data_Interface {
	...
	public function Get_Report_Summary_Data()
	{
		...
		$titles_and_queries = array(
			...
			"7 Day Task Averages (Hours/Day):" => "SELECT `tasks`.`name` AS `name`, (SUM(`task_log`.`hours`) / 7) AS `agg_value` 
			FROM `task_log`, `tasks` 
			...
			"All Time Item Sums:" => "SELECT `unit` AS `name`, (SUM( `value` )) AS `agg_value` 
			FROM `item_log` , `items`
			WHERE `item_log`.`item_id` = `items`.`item_id`
			GROUP BY `unit`",
			
			"All Time Task Sums:" => "SELECT `tasks`.`name` AS `name`, (SUM(`task_log`.`hours`)) AS `agg_value` 
			FROM `task_log`, `tasks` 
